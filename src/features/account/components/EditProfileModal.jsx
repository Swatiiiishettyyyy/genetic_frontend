...
export function EditProfileModal({ profile, mode = "edit", lockRelationship = false, onClose, onSave }) {
  ...
  return (
    ...
    >
      <div className="h-full w-full overflow-y-auto bg-[#F5F0FF] sm:max-h-[calc(100dvh-2rem)] sm:max-w-[32rem] sm:rounded-[1.25rem] sm:bg-white sm:p-[clamp(1.25rem,2.5vw,1.75rem)] sm:shadow-nucleotide">
        <form onSubmit={handleSubmit} className="flex min-h-full flex-col sm:block">
          ...
          <div className="flex items-start justify-between gap-4 px-4 pb-3 pt-8 sm:px-0 sm:pb-0 sm:pt-0">
            ...
            <button
              ...
              onClick={onClose}
              aria-label="Close"
              className="hidden size-5 shrink-0 text-nucleotide-muted transition hover:text-nucleotide-ink sm:block"
            >
              <CloseIcon />
            </button>
          </div>
          ...
          <div className="mt-auto flex items-center justify-end gap-6 bg-white px-4 py-4 sm:mt-5 sm:bg-transparent sm:p-0">
            <button
              type="button"
              onClick={onClose}
