...
export function menHealthPath(age: ComprehensiveAgeBand): string {
  ...
  return row ? bloodTestPath(`men-health/${row.slug}`) : bloodTestPath('men-health/under-25')
}
...
export default function MenHealthSegmentPage({ cartCount }: { cartCount?: number }) {
  ...
  useEffect(() => {
    if (!active) navigate(bloodTestPath('men-health/under-25'), { replace: true })
  }, [active, navigate])
  ...
  const cards = useMemo(
    () => active ? filterComprehensive(products, 'men', active.age).map((p, i) => ({
      ...toTestCard(p),
      analyticsListName: `Men Health ${active.label}`,
      analyticsListId: `BT_MEN_HEALTH_${active.slug.toUpperCase().replace(/[^A-Z0-9]+/g, '_')}`,
      analyticsIndex: i + 1,
    })) : [],
    [products, active],
  )
        ...
        <div style={OVERLAY} onClick={() => setShowGenderModal(false)}>
          ...
          >
            ...
            <div style={{ paddingInline: 25, display: 'flex', flexDirection: 'column', gap: 18 }}>
              ...
              <button
                type="button"
                onClick={() => navigate(bloodTestPath(`women-health/${active.slug}`))}
                style={{
                  border: 'none',
                  background: 'transparent',
                  padding: 0,
              ...
              {AGE_SEGMENTS.map(s => {
                ...
                return (
                  <div key={s.slug} style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                    <button
                      type="button"
                      onClick={() => { setShowAgeModal(false); navigate(bloodTestPath(`men-health/${s.slug}`)) }}
                      style={{
                        border: 'none',
                        background: 'transparent',
                        padding: 0,
