...
import { familyProfiles } from "../../../shared/data/accountProfile.js";
...
function SwitchProfileDropdown({ onClose }) {
  return (
    ...
    >
        ...
        {familyProfiles.map((member) => (
          ...
          >
            <div className="flex items-center gap-[clamp(0.75rem,1.2vw,1rem)]">
              <Avatar initials={member.initials} size="sm" className="size-12 shrink-0 text-[1rem]" />
      ...
      >
        <div className="flex size-12 shrink-0 items-center justify-center rounded-full border border-dashed border-[rgba(136,107,249,0.5)]">
          <PlusIcon />
        </div>
        <span className="font-poppins text-[clamp(0.875rem,0.9vw,1.125rem)] font-medium leading-[1.3] text-nucleotide-ink">
          Add Family Member
            ...
            <div className="space-y-2">
              {section.items.map((item) => (
                <a
                  ...
                  className={cn(
                    "flex min-h-9 items-center gap-3 rounded-[10px] px-2 py-1.5 font-inter text-[clamp(0.875rem,0.78rem+0.34vw,1rem)] leading-[1.5] text-nucleotide-ink transition hover:bg-nucleotide-lavender/30 hover:text-nucleotide-purple",
                    item.label === activeLabel && "bg-nucleotide-purple/10 text-nucleotide-purple",
                  )}
